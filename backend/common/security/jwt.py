...
def get_token(request: Request) -> str:
    ...
    if not authorization or scheme.lower() != "bearer":
        raise TokenError(msg="Token недействительный")
    return token
...
def jwt_decode(token: str) -> int:
    ...
    try:
        ...
        raise TokenError(msg="Token Истекший")
...
async def get_current_user(db: AsyncSession, pk: int) -> User:
    ...
    if user.dept_id:
        if not user.dept.status:
            raise AuthorizationError(msg="Отдел пользователя заблокирован")
        if user.dept.del_flag:
            raise AuthorizationError(msg="Отдел пользователя был удален")
    if user.roles:
        role_status = [role.status for role in user.roles]
        if all(status == 0 for status in role_status):
            raise AuthorizationError(
                msg="Роль, к которой принадлежит пользователь, заблокирована"
            )
    return user

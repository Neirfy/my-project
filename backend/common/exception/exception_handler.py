...
def _get_exception_code(status_code: int):
    """
    Код статуса возврата, OpenAPI, Uvicorn.
...
async def _validation_exception_handler(
    ...
    """
    Обработка исключений при проверке данных
    ...
    if error.get("type") == "json_invalid":
        message = "не удалось выполнить синтаксический анализ json"
    else:
        error_input = error.get("input")
        field = str(error.get("loc")[-1])
        error_msg = error.get("msg")
        message = (
            f"{field} {error_msg}，ввод：{error_input}"
            if settings.ENVIRONMENT == "dev"
            else error_msg
        )
    msg = f"Параметр запроса является недопустимым: {message}"
...
def register_exception(app: FastAPI):
    ...
    if settings.MIDDLEWARE_CORS:
        ...
        async def cors_custom_code_500_exception_handler(request, exc):
            """
            Междоменная пользовательская обработка 500 исключений

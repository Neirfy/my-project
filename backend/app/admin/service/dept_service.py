from typing import Any

from app.admin.crud.crud_dept import dept_dao
from app.admin.model.sys_dept import Dept
from app.admin.schema.dept import CreateDeptParam, UpdateDeptParam
from common.exception import errors
from database.db_postgres import async_session
from utils.build_tree import get_tree_data


class DeptService:
    @staticmethod
    async def get(*, pk: int) -> Dept:
        async with async_session() as db:
            dept = await dept_dao.get(db, pk)
            if not dept:
                raise errors.NotFoundError(msg="Этого отдела не существует")
            return dept

    @staticmethod
    async def get_dept_tree(
        *,
        name: str | None = None,
        leader: str | None = None,
        phone: str | None = None,
        status: int | None = None
    ) -> list[dict[str, Any]]:
        async with async_session() as db:
            dept_select = await dept_dao.get_all(
                db=db, name=name, leader=leader, phone=phone, status=status
            )
            tree_data = get_tree_data(dept_select)
            return tree_data

    @staticmethod
    async def create(*, obj: CreateDeptParam) -> None:
        async with async_session.begin() as db:
            dept = await dept_dao.get_by_name(db, obj.name)
            if dept:
                raise errors.ForbiddenError(msg="Название отдела уже существует")
            if obj.parent_id:
                parent_dept = await dept_dao.get(db, obj.parent_id)
                if not parent_dept:
                    raise errors.NotFoundError(msg="Родительский отдел не существует")
            await dept_dao.create(db, obj)

    @staticmethod
    async def update(*, pk: int, obj: UpdateDeptParam) -> int:
        async with async_session.begin() as db:
            dept = await dept_dao.get(db, pk)
            if not dept:
                raise errors.NotFoundError(msg="Этого отдела не существует")
            if dept.name != obj.name:
                if await dept_dao.get_by_name(db, obj.name):
                    raise errors.ForbiddenError(msg="Название отдела уже существует")
            if obj.parent_id:
                parent_dept = await dept_dao.get(db, obj.parent_id)
                if not parent_dept:
                    raise errors.NotFoundError(msg="Родительский отдел не существует")
            if obj.parent_id == dept.id:
                raise errors.ForbiddenError(
                    msg="Запретить ассоциировать себя с родителями"
                )
            count = await dept_dao.update(db, pk, obj)
            return count

    @staticmethod
    async def delete(*, pk: int) -> int:
        async with async_session.begin() as db:
            dept_user = await dept_dao.get_with_relation(db, pk)
            if dept_user:
                raise errors.ForbiddenError(
                    msg="В этом отделе есть пользователи, которые не могут быть удалены"
                )
            children = await dept_dao.get_children(db, pk)
            if children:
                raise errors.ForbiddenError(
                    msg="В рамках департамента есть подотделы, которые нельзя удалить"
                )
            count = await dept_dao.delete(db, pk)
            return count


dept_service: DeptService = DeptService()
